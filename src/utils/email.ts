...
const sendVerifyEmail = (toAddress: string, subject: string, body: string) => {
  const sendEmailCommand = createSendEmailCommand({
    fromAddress: EnvConfig.AWS_SES_FROM_ADDRESS as string,
...
export const sendVerifyRegisterEmail = (
  ...
  return sendVerifyEmail(
    ...
    template
      ...
      .replace('{{link}}', `${EnvConfig.CLIENT_BASE_URL}/email-verifications?token=${email_verify_token}`)
  )
}
...
export const sendForgotPasswordEmail = (
  ...
  return sendVerifyEmail(
    ...
    template
      ...
      .replace('{{link}}', `${EnvConfig.CLIENT_BASE_URL}/forgot-password?token=${forgot_password_token}`)
  )
}
